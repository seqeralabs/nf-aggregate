...
- **Session labels are confirmed present in the real SciDev Intelligent Compute export** (`s3://intelligent-compute-testing-cost/.../SIC-testing-costs/data/`, account 677974454204, 38 distinct tag keys): `user_nextflow_io_session_id` on 34,443 rows (scheduler-tagged instances) and `user_pipeline_session_id` on 8,662 (the Batch label template). A third, shorter variant — `user_workflow_id`/`user_session_id`, 76 rows, $1.07 of ECS split cost — matched none of the aliases and was silently dropped, so both are now tried last. Validated end to end on a real resumed run: `5JpjMefy6nK5dQ` (nf-core/molkart, `-resume 65d66f4c-…`, 31 cached / 6 executed) reports `attempts=2`, `cost=$0.0295` for its own attempt and `session_cost=$0.343` across the session — matching a `GROUP BY` straight off the parquet ($0.3135 for attempt `4DAIUmnBrJTZBG` + $0.0295). An 11.6x gap between what that run's own workflow id was billed and what the pipeline actually spent, which is the whole point of the feature.
...
- **Why undeclared buckets stopped working on Intelligent Compute (provenance).** `seqeralabs/sched#1015` — *"fix(iam): scope the cluster-minted S3 policy to the compute env allowBuckets"*, merged 2026-08-12T15:54:42Z, first tagged in `v0.79.2` (2026-08-15) — replaced the cluster-minted role's unconditional `"Resource": "*"` with the compute environment's `allowBuckets` list, and renamed the per-cluster instance profile `seqera-sched-instance-v1-{clusterId}` → **`v2`** (which is how the role in a failure log dates itself). Before it, `RequirementsServiceImpl:587` hard-coded an empty bucket set, so every cluster role held account-wide S3 — measured in prod on the PR: 1545 clusters wildcard, 0 scoped. That is why an nf-aggregate run on 2026-08-12 at 20:00 read a CUR export from an **undeclared** bucket (`--costs data`, exit 0, 6.3 MB) and the identical command failed on 2026-08-21 with `403 AccessDenied` on `s3:ListBucket`. Per that PR a CE declaring nothing keeps the wildcard, so only compute environments that *do* declare buckets lose undeclared ones. Tracking issue `sched#1013` is still open (this was part 1 of 4). Secondary, unrelated to the denial: Fusion v2.6.3 (`fusion#1617`, 2026-08-14) reached sched via `sched#1117` (2026-08-18), which is why these tasks now log NFS service mode.
- **A CUR read failure is almost always the compute environment's bucket list, and a single file does not dodge it.** Fusion resolves every path by listing its parent prefix (`Lookup` -> `PopulateDirectory` -> `ListObjectsV2`), so `s3:ListBucket` on the bucket is required even when the parameter names one `*.parquet`. Traced from a real failure (`3NReMYELu0O7uI`): the task's `.fusion.server.log` — kept in the work directory, and the single most useful artifact for this class — recorded `403 AccessDenied ... not authorized to perform: s3:ListBucket` followed by *"Fusion authenticated successfully but lacks permission to access this resource"*. On Intelligent Compute the acting identity is an **ephemeral per-cluster instance role** (`seqera-sched-instance-v2-clu-<id>`, created at cluster start) whose inline policy is derived from the compute environment's `allowBuckets`; the CE's own Forge instance role (`TowerForge-<ce>-InstanceRoleCloud`) grants the same list. So a bucket outside `allowBuckets` is denied however the path is written — the fix is to add it to the compute environment, not to reshape the path. Note the same run's other directory input staged from the *work* bucket read fine, which is how the two are told apart.
