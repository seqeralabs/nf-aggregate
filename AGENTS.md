...
- **Intelligent Compute report cost basis.** The IC report (`benchmark_report_ic_aggregate.py`) prefers ECS split cost allocation per run: if a run's CUR rows carry genuine split cost (`split_cost_present`), `run_summary[].cost_basis` is `split` with `used_cost`/`unused_cost` (used vs idle capacity); otherwise it falls back to `blended` and both are null. `cost` is always the billed total. It mirrors the benchmark report's `cost_status` (`available`/`propagating`/`not_found`/null). `normalize` emits `split_cost_present` (shared with the benchmark path). The report shows per-run cost-basis badges plus a Cost-section coverage note (whether IC used split vs blended, and how many runs had costs).
- `commit.gpgsign` must be true (SSH signing via 1Password)
