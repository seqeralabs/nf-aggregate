...
1. **API path:** `SeqeraApi.fetchRunData()` runs in Groovy process memory (not a Nextflow task). It first validates the configured Platform endpoint with `/service-info` and the bearer token with `/user-info`, then resolves workspace name → ID and fetches `/workflow/{id}`, `/workflow/{id}/metrics`, `/workflow/{id}/tasks` (paginated), and `/workflow/{id}/progress`. It returns `{workflow, schedEnabled, schedConfig, platform, metrics, tasks, progress}` — `schedEnabled`/`schedConfig`/`platform` are pulled from the `/workflow/{id}` payload for the Intelligent Compute report. Results are written to temp JSON files.
